...
{
    public class QueryValueDiff2PointsAttributeValueBlockExecution : BaseBlockExecution
    {
        public QueryValueDiff2PointsAttributeValueBlockExecution(IBlockExecution next,
                                                    IServiceProvider serviceProvider) : base(next, serviceProvider)
        {
        }

        protected override BlockOperator CurrentBlockOperator => BlockOperator.DifferenceValueBetween2PointSingleAttributeValue;
        ...
        protected override async Task<BlockQueryResult> ExecuteOperationAsync(IBlockContext context)
        {
            using (var scope = _serviceProvider.CreateNewScope())
            {
                ...
                var result = await blockExecutionRepository.GetValueDiff2PointsAssetAttributeValueAsync(assetId, assetAttributeId, context.BlockOperation.StartTime, context.BlockOperation.EndTime);
