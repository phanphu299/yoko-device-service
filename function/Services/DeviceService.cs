...
namespace AHI.Device.Function.Services
{
    public class DeviceService : IDeviceService
    {
        ...
        public static readonly string[] RESERVE_KEYS = new[] { "tenantId", "subscriptionId", "projectId", "deviceId", "integrationId" };
        public DeviceService(IDbConnectionFactory dbConnectionFactory,
        ...
        public async Task<(long, DateTime?, IEnumerable<Guid>)> ProcessEventAsync(IDictionary<string, object> metricDict)
        {
            ...
            var (deviceId, timestamp, updatedUtc, metrics) = await UpdateDeviceSnapshotAsync(projectId, metricDict);
        ...
        private async Task<(string, long, DateTime?, string[])> UpdateDeviceSnapshotAsync(string projectId, IDictionary<string, object> metricDict)
        {
            ...
            var hashField = CacheKey.PROCESSING_DEVICE_HASH_FIELD.GetCacheKey($"metric_{string.Join("_", metricDict.Select(x => $"{x.Key}{x.Value}"))}".CalculateMd5Hash(), "");
            var hashKey = CacheKey.PROCESSING_DEVICE_HASH_KEY.GetCacheKey(projectId);

            var cacheHit = await _cache.GetHashByKeyInStringAsync(hashKey, hashField);
            ...
            await _cache.SetHashByKeyAsync(hashKey, hashField, "cached", TimeSpan.FromHours(2));
            ...
            {
                _logger.LogError($"Possible DeviceIds {string.Join(",", deviceIds)} not found!. Project: {projectId}!");
                return (null, 0, null, null);
            }
            ...
            if (deviceInformation.EnableHealthCheck)
            {
                ...
                {
                    _logger.LogError(exc, $"[{activityId}] UpdateDeviceSnapshotAsync - TrackingHeartbeatAsync failed - keep processing...");
                }
            }
            ...
            {
                _logger.LogError(e, $"[{activityId}] UpdateDeviceSnapshotAsync - Parsing Snapshot value failed!");
                return (null, 0, null, null);
            }
